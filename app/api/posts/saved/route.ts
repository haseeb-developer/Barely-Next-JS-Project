...
// GET - Fetch all saved posts for the current user
export async function GET(request: NextRequest) {
  try {
        ...
        postsWithCounts = postsWithCounts.map((p: any) => ({
          ...
          gif_profile_enabled:
            p.user_type === "anonymous"
              ...
              : false,
          saved: true, // All posts in saved tab are saved
          liked: false, // Will be set below
    ...
    // Batch fetch liked/disliked status for saved posts
    try {
      ...
      console.error("Error fetching liked/disliked status:", error);
    }

    return NextResponse.json({ posts: postsWithCounts });
  } catch (e: any) {
    console.error("Error in GET saved posts:", e);
    return NextResponse.json(
