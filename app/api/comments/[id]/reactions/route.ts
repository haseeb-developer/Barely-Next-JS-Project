...
export async function GET(
  ...
  try {
    ...
    console.error("GET comment reactions error:", e);
...
export async function POST(
  ...
  try {
    ...
    if (existing) {
      ...
      const { error: insErr } = await supabase.from("comment_reactions").insert({ comment_id: commentId, emoji, user_id, user_type });
    ...
    console.error("POST comment reactions error:", e);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
