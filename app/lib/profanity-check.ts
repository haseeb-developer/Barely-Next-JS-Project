// Profanity checking utility - can be used for usernames, posts, and any text content
...
// Check if text contains any prohibited word (with obfuscation detection)
...
// Main profanity check function with multiple layers
export const checkProfanity = async (text: string): Promise<boolean> => {
  ...
  // Layer 6: Final check for character substitution patterns
  const normalized = normalizeText(text);
